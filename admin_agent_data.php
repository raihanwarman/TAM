...
<nav class="w3-sidebar w3-collapse w3-white w3-animate-left" style="z-index:3;width:300px;" id="mySidebar"><br>
  ...
  <div class="w3-bar-block">
    ...
    <a href="#" class="w3-bar-item w3-button w3-padding w3-blue"><i class="fa fa-users fa-fw"></i> Agent Data</a>
       ...
       <?php
         ...
         if (mysqli_num_rows($result) > 0) {
           while($row = $result->fetch_assoc()) {
             echo"<tr>";
             echo"<td>".$row["nama"]."</td>";
             echo'<td><a href="quality_agent_data_preview.php?usr='.$row["username"].'"'. 'class="w3-button w3-padding "><i class="fa fa-arrow-right fa-fw"></i></a><small></small></td>';
             echo"</tr>";
           }
         }
       ?>
     ...
     <div class="w3-half">
       ...
       <table class="w3-table-all">
        ...
        <?php
          $query="select user.username as username, user.nama as nama from
                  (user inner join agent on user.username = agent.username)
                  where agent.position = 'outbound' order by nama ASC";
          $result = mysqli_query($conn, $query);
          //tulis output di tabel
          if (mysqli_num_rows($result) > 0) {
            while($row = $result->fetch_assoc()) {
